...
import { evaluateRobotsAccess, parseCrawlDelay } from "@/features/geo-audit/engine/robots";
import { extractJsonLdItems, structuredDataCheck } from "@/features/geo-audit/engine/checks/schema-authority";
import { canonicalCheck } from "@/features/geo-audit/engine/checks/crawl-signals";
import { runGeoAudit } from "@/features/geo-audit/engine/orchestrator";
import type { CheckCtx } from "@/features/geo-audit/engine/checks/ctx";
import dns from "node:dns/promises";

describe("normalizeSiteUrl", () => {
  it("accepts a bare domain and treats it as https homepage", () => {
    const r = normalizeSiteUrl("example.com");
    expect(r.ok).toBe(true);
    expect(r.url).toBe("https://example.com/");
  });

  it("accepts a full URL with a path", () => {
    const r = normalizeSiteUrl("https://example.com/blog/post");
    expect(r.ok).toBe(true);
    expect(r.url).toBe("https://example.com/blog/post");
  });

  it("accepts uppercase and http", () => {
    const r = normalizeSiteUrl("HTTP://EXAMPLE.com/About");
    expect(r.ok).toBe(true);
    expect(r.url).toBe("http://example.com/About");
  });

  it("rejects IPs, localhost and private names", () => {
    expect(normalizeSiteUrl("192.168.1.1").ok).toBe(false);
    expect(normalizeSiteUrl("http://localhost:3000/x").ok).toBe(false);
    expect(normalizeSiteUrl("http://10.0.0.5").ok).toBe(false);
    expect(normalizeSiteUrl("mail.internal").ok).toBe(false);
  });

  it("rejects non-http schemes and junk", () => {
    expect(normalizeSiteUrl("ftp://example.com").ok).toBe(false);
    expect(normalizeSiteUrl("not a url at all").ok).toBe(false);
    expect(normalizeSiteUrl("").ok).toBe(false);
  });
});
