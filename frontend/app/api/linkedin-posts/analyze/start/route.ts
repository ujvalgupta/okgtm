...
import { verifyOtp } from "@/lib/linkedin-posts/otp";
import { normalizeLinkedInUrl } from "@/lib/linkedin-posts/profile-url";
import { startPostsJob } from "@/lib/linkedin-posts/mindcase";
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const jobId = await startPostsJob(normalizedUrl, LINKEDIN_POSTS_CONFIG.maxResults);
