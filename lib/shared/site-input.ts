/**
 * Website-URL input normalization shared by every tool that audits a site by
 * URL (GEO audit, llms.txt validator, …). Accepts a full URL or a bare domain
 * (treated as https://<domain>/). Rejects IPs, localhost, private/internal
 * names, and non-http(s) schemes.
 */
...
export function normalizeSiteUrl(raw: string): SiteInput {
  let s = (raw ?? "").trim();
